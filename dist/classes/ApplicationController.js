...
export class ApplicationController {
    ...
    bootstrapDemoData() {
        ...
        for (const { title, transactions } of demo) {
            ...
            for (const row of transactions) {
                account.addTransaction(new Transaction(row.amount, row.type, row.date, row.description));
            }
    ...
    showAccountsOverview() {
        ...
        const accountsMarkup = accounts.length === 0
    ...
    attachOverviewHandlers(container) {
    ...
    `;
        ...
        const cancelButton = container.querySelector('[data-action="cancel-create-account"]');
        if (form) {
            form.onsubmit = event => {
    ...
    renderAccountDetails(account) {
        ...
        const transactionsMarkup = transactions.length === 0
    ...
    attachAccountDetailsHandlers(container, accountId) {
        container.onclick = event => {
            const target = event.target;
            if (!target)
                return;
    ...
    showAddTransactionForm(accountId) {
        const account = this.accountManager.getAccount(accountId);
        if (!account)
            return;
        const container = this.getRootContainer();
    ...
    `;
        ...
        const cancelButton = container.querySelector('[data-action="cancel-add-transaction"]');
        if (form) {
            form.onsubmit = event => {
                event.preventDefault();
    ...
    handleAddTransactionSubmit(accountId) {
        const account = this.accountManager.getAccount(accountId);
        if (!account)
            return;
    ...
    confirmAndDeleteAccount(accountId) {
        const confirmed = confirm('Вы уверены, что хотите удалить этот счёт? Все транзакции будут удалены.');
        if (!confirmed) {
            return;
        }
    ...
    `;
        ...
        const cancelButton = container.querySelector('[data-action="cancel-export"]');
        if (form) {
            form.onsubmit = event => {
                event.preventDefault();
